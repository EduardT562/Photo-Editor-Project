...
PixelData.prototype.getNeighbours = function (x, y) {
  ...
  let result = [];
  for (i = 0; i < neighboursPositions.length; i++) {
    if (
      neighboursPositions[i][0] === -1 ||
      neighboursPositions[i][0] >= this.height ||
      neighboursPositions[i][1] === -1 ||
      neighboursPositions[i][1] >= this.width
    ) {
      result[i] = new Pixel(0, 0, 0);
    } else {
      result[i] =
        this.pixels[neighboursPositions[i][0]][neighboursPositions[i][1]];
    }
  }
